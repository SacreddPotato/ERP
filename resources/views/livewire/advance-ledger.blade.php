<div class="space-y-4">
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <h2 class="text-lg font-semibold mb-4">{{ __('advance_entry') }}</h2>
        <div class="flex items-end gap-3 mb-4">
            <div class="flex-1">
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('advance_id') }}</label>
                <input type="text" wire:model="entityCode" placeholder="{{ __('placeholder_advance_id') }}"
        ...
        <h3 class="text-sm font-medium text-gray-600 mb-3">{{ __('new_advance_details') }}</h3>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-lg font-semibold">{{ __('advances_list') }}</h2>
            ...
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-3 py-2 font-mono">{{ $entity->advance_code }}</td>
                        ...
                        <td class="px-3 py-2 whitespace-nowrap">
                            <button wire:click="showTransactions('{{ $entity->advance_code }}')" class="text-blue-600 hover:underline text-xs">{{ __('tab_transactions') }}</button>
                            <button wire:click="requestDelete('{{ $entity->advance_code }}', '{{ $entity->employee_name }}')" class="text-red-600 hover:underline text-xs ms-1">{{ __('btn_delete') }}</button>
                        </td>
                    </tr>
                    @if ($viewingTransactions === $entity->advance_code)
                    <tr>
                    ...
                    @endif
                    @empty
                    <tr><td colspan="9" class="px-3 py-8 text-center text-gray-400">{{ __('no_advances') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        <p class="text-xs text-gray-400 mt-2">{{ __('showing_advances', ['count' => $entities->count()]) }}</p>
    </div>
</div>
