<div class="space-y-4">
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <h2 class="text-lg font-semibold mb-4">{{ __('customer_entry') }}</h2>
        <div class="flex items-end gap-3 mb-4">
            <div class="flex-1">
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('customer_id') }}</label>
                <input type="text" wire:model="entityCode" placeholder="{{ __('placeholder_customer_id') }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('customer_name') }} *</label>
                <input type="text" wire:model="name" placeholder="{{ __('placeholder_customer_name') }}"
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-lg font-semibold">{{ __('customers_list') }}</h2>
            ...
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-3 py-2 font-mono">{{ $entity->customer_code }}</td>
                        ...
                        <td class="px-3 py-2 whitespace-nowrap">
                            <button wire:click="showTransactions('{{ $entity->customer_code }}')" class="text-blue-600 hover:underline text-xs">{{ __('tab_transactions') }}</button>
                            <button wire:click="requestDelete('{{ $entity->customer_code }}', '{{ $entity->name }}')" class="text-red-600 hover:underline text-xs ms-1">{{ __('btn_delete') }}</button>
                        </td>
                    </tr>
                    @if ($viewingTransactions === $entity->customer_code)
                    <tr>
                    ...
                    @endif
                    @empty
                    <tr><td colspan="10" class="px-3 py-8 text-center text-gray-400">{{ __('no_customers') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        <p class="text-xs text-gray-400 mt-2">{{ __('showing_customers', ['count' => $entities->count()]) }}</p>
    </div>
</div>
