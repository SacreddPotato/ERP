<div class="space-y-4">
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <h2 class="text-lg font-semibold mb-4">{{ __('covenant_entry') }}</h2>
        <div class="flex items-end gap-3 mb-4">
            <div class="flex-1">
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('covenant_id') }}</label>
                <input type="text" wire:model="entityCode" placeholder="{{ __('placeholder_covenant_id') }}"
        ...
        <h3 class="text-sm font-medium text-gray-600 mb-3">{{ __('new_covenant_details') }}</h3>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('employee_name') }} *</label>
                <input type="text" wire:model="employeeName" placeholder="{{ __('placeholder_employee_name') }}"
                    class="w-full rounded-md border-gray-300 shadow-sm text-sm" {{ $isExisting ? 'disabled' : '' }}>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ __('employee_phone') }}</label>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-lg font-semibold">{{ __('covenants_list') }}</h2>
            ...
            <table class="min-w-full text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-start">{{ __('employee_name') }}</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-3 py-2 font-mono">{{ $entity->covenant_code }}</td>
                        <td class="px-3 py-2">{{ $entity->employee_name }}</td>
                        ...
                        <td class="px-3 py-2 whitespace-nowrap">
                            <button wire:click="showTransactions('{{ $entity->covenant_code }}')" class="text-blue-600 hover:underline text-xs">{{ __('tab_transactions') }}</button>
                            <button wire:click="requestDelete('{{ $entity->covenant_code }}', '{{ $entity->employee_name }}')" class="text-red-600 hover:underline text-xs ms-1">{{ __('btn_delete') }}</button>
                        </td>
                    </tr>
                    @if ($viewingTransactions === $entity->covenant_code)
                    <tr>
                        <td colspan="9" class="px-3 py-3 bg-gray-50">
                            <h4 class="text-sm font-semibold mb-2">{{ __('transaction_log') }} - {{ $entity->employee_name }}</h4>
                    ...
                    @endif
                    @empty
                    <tr><td colspan="9" class="px-3 py-8 text-center text-gray-400">{{ __('no_covenants') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        <p class="text-xs text-gray-400 mt-2">{{ __('showing_covenants', ['count' => $entities->count()]) }}</p>
    </div>
</div>
