...
class CovenantLedger extends Component
{
    ...
    protected LedgerType $ledgerType = LedgerType::Covenant;
    ...
    {
        if (empty($this->entityCode) || empty($this->employeeName)) {
    ...
    {
        $this->dispatch('show-delete-modal', type: 'covenant', code: $code, name: $name);
    }

    #[On('delete-confirmed')]
    public function handleDelete(string $type, string $code, string $password): void
    {
        if ($type !== 'covenant') return;
        try {
            $service = app(LedgerService::class);
    ...
    public function render()
    {
        ...
        return view('livewire.covenant-ledger', [
            'entities' => $entities,
            'totals' => $totals,
